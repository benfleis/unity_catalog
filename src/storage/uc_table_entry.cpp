		...
		if (plan.status == UCScanPlanStatus::COMPLETED) {
			...
			if (!plan.storage_credentials.empty()) {
				...
				for (idx_t i = 0; i < plan.storage_credentials.size(); i++) {
					...
					sec.name = "__internal_uc_scanplan__" + bd.catalog_name + "__" + bd.schema_name + "__" +
			...
			auto &parquet_entry = sys_cat.GetEntry<TableFunctionCatalogEntry>(context, DEFAULT_SCHEMA, "parquet_scan");
