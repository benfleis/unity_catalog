...
if ! uc schema get --full_name duck.cmt >/dev/null 2>&1; then
	...
	echo "duck-entrypoint: created schema 'duck.cmt' -> $CMT_ROOT"
fi
...
if ! uc schema get --full_name duck.plain >/dev/null 2>&1; then
	...
	echo "duck-entrypoint: created schema 'duck.plain' (plain tables under $PLAIN_ROOT)"
fi

echo "duck-entrypoint: ready. catalog 'duck' with schemas 'cmt' and 'plain'."
