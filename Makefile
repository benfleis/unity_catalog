...
# teardown). This overrides extension-ci-tools' `test` (which runs the binary directly and would
# bypass the provisioner). OSS runs against the ducklabs container; databricks skips without
# creds -- use `make test_databricks` (wraps run_databricks_env) for the credentialed run.
.PHONY: test test_databricks
test:
